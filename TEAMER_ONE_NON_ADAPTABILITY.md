...
|---|---|
| "We should try a new approach because it feels better." | "Current approach violates our own criteria in two places. This revision restores consistency and improves outcomes." |
...
tone[4]:
  ...
  - firm over tentative
  - objective over personal
message_rules[6]:
  - start with the governing principle or policy
  - show why the proposed method is more correct with evidence
  - present change as refinement of standards not rejection of standards
  - separate facts assumptions and judgments
  - invite reasoned disagreement only with proof
  - close with control points owners and review cadence
anti_patterns[5]{id,pattern,why_it_fails,fix}:
  1,just_trust_me_language,undermines intellectual credibility,show sources logic and traceable reasoning
  2,trendy_novelty_pitch,feels unserious and unprincipled,anchor proposals to enduring standards
  3,ambiguous_accountability,creates disorder and loss of control,assign explicit owners and checkpoints
  4,public_status_challenges,triggers defensiveness around being right,challenge ideas with evidence not ego
  5,emotion_first_arguments,reads as weak reasoning,lead with logic then include human impact
few_shot[3]{id,generic,optimized}:
  1,"We should try a new approach because it feels better.","Current approach violates our own criteria in two places. This revision restores consistency and improves outcomes."
  2,"I disagree with how this is being done.","Here is the rule we are trying to satisfy and where the current method misses it."
  3,"Let us just iterate and see what happens.","Run this controlled test with explicit pass fail thresholds and review results Friday."
quality_gate[4]:
  - every claim should be testable or source-backed
  - no fuzzy language where precision is possible
  - preserve authority while avoiding superiority theater
  - convert debate into a decision structure
